...
public class ServerHelper {
    ...
    public static void report(String event,CallBack<ReportResponseData> cb){
        if(UserManager.getInstance().getUser()==null){
            if(cb!=null) {
                cb.onFailed(new Throwable("user is null"));
            }
            return;
        }
        ...
        ApiManager.getInstance()
            .requestReportAction(UserManager.getInstance().getUser().userNo, "agora_labs",
                AESUtils.getS1(App.getInstance()), "agora_labs", event)
            ...
            .subscribe(new ApiSubscriber<BaseResponse<BaseRes>>() {
                @Override public void onSuccess(BaseResponse<BaseRes> baseResBaseResponse) {
                    //Log.d("AgoraLab","ApiManager---requestReportAction---success");
                }

                @Override public void onFailure(ApiException t) {
                    //Log.d("AgoraLab","ApiManager---requestReportAction---onFailure："+ GsonUtils.getGson().toJson(t));
                }
            });
    }
