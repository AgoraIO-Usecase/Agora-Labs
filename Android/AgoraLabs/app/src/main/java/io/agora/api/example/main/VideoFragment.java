...
public class VideoFragment extends BaseListFragment {
    ...
    private final int ID_TRANSPARENT_BACKGROUND_TRANSFER=12;
    ...
    protected void initData(){
        ...
        data.add(new Feature(ID_TRANSPARENT_BACKGROUND_TRANSFER,R.mipmap.split_green_screen,R.string.transparent_background_transfer,true));
    ...
    @Override protected void onItemSelect(Feature feature, int position) {
        ...
        }else if(feature.getId()==ID_TRANSPARENT_BACKGROUND_TRANSFER){
            NavController navController = Navigation.findNavController(getActivity(), R.id.nav_host_fragment);
            navController.navigate(R.id.action_homeFragment_to_transparent_bg_transfer);
            reportEvent("TransparentBackgroundTransfer");
        }
    }
