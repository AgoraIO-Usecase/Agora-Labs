...
import io.agora.api.example.databinding.FragmentLighting3dBinding;
...
public class Lighting3DFragment extends Fragment implements View.OnClickListener{
    private String TAG="metakitx";
    protected RtcEngine rtcEngine;
    protected int sceneMode= Constants.CHANNEL_PROFILE_LIVE_BROADCASTING;
    private FragmentLighting3dBinding binding;

    private final int ID_LIGHT_3D=0;
    private final int ID_LIGHTING_MOOD=1;
    private final int ID_LIGHT_AD=2;
    ...
    @Nullable @Override public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
        ...
        binding= FragmentLighting3dBinding.inflate(inflater,container,false);
    ...
    private void enableAi(boolean on){
        ...
        if(on){
            ...
            metaEngineHandler.configMetaBackgroundEffectMode(effectId,false);
        }
    }

    private boolean isEffectModeSetPending=false;
    private void updateAi() {
        Log.d(TAG,"updateAi:"+aiEnabled+" effectId:"+effectId+" isEffectModeSetPending:"+isEffectModeSetPending);
        if(!aiEnabled){
            return;
        }
    ...
    public void setEffectMode(){
        if(effectId==MetaEngineHandler.SpecialEffectType.SETypeAmbLight){
            metaEngineHandler.configMetaBackgroundEffectMode(MetaEngineHandler.SpecialEffectType.SETypeAdvLight,false);
            metaEngineHandler.configMetaBackgroundEffectMode(MetaEngineHandler.SpecialEffectType.SEType3DLight,false);
            metaEngineHandler.configMetaBackgroundEffectMode(MetaEngineHandler.SpecialEffectType.SETypeAmbLight,true);
        }else if(effectId==MetaEngineHandler.SpecialEffectType.SETypeAdvLight){
            metaEngineHandler.configMetaBackgroundEffectMode(MetaEngineHandler.SpecialEffectType.SEType3DLight,false);
            metaEngineHandler.configMetaBackgroundEffectMode(MetaEngineHandler.SpecialEffectType.SETypeAmbLight,false);
            metaEngineHandler.configMetaBackgroundEffectMode(MetaEngineHandler.SpecialEffectType.SETypeAdvLight,true);
        }else if(effectId==MetaEngineHandler.SpecialEffectType.SEType3DLight){
            metaEngineHandler.configMetaBackgroundEffectMode(MetaEngineHandler.SpecialEffectType.SEType3DLight,false);
            metaEngineHandler.configMetaBackgroundEffectMode(MetaEngineHandler.SpecialEffectType.SETypeAdvLight,false);
            metaEngineHandler.configMetaBackgroundEffectMode(MetaEngineHandler.SpecialEffectType.SEType3DLight,true);
        }
    }
    ...
    private void initView(){
        ...
        data.add(new OptionItem(ID_LIGHT_3D,R.mipmap.lighting_3d,R.string.lighting_3d));
        data.add(new OptionItem(ID_LIGHTING_MOOD,R.mipmap.mood_lighting,R.string.lighting_mood));
        data.add(new OptionItem(ID_LIGHT_AD,R.mipmap.ad_lighting,R.string.lighting_ad));
    ...
    private void onMenuItemSelected(OptionItem item,int position){
        SystemUtil.vibrator(getContext());
        if(item.getId()==ID_LIGHT_3D){
            effectId= MetaEngineHandler.SpecialEffectType.SEType3DLight;
        }else if(item.getId()==ID_LIGHTING_MOOD){
            effectId= MetaEngineHandler.SpecialEffectType.SETypeAmbLight;
        }else if(item.getId()==ID_LIGHT_AD){
            effectId= MetaEngineHandler.SpecialEffectType.SETypeAdvLight;
        }
        updateAi();
