...
public class ApiManager {
    ...
    public Observable<BaseResponse<BaseRes>> requestReportDevice(String userNo,String sceneId,String appId,
        ...
        return apiManagerService.requestReportDevice(userNo,sceneId,appId,projectId,getRequestBody(params)).flatMap(it -> Observable.just(it));
    }

    public Observable<BaseResponse<BaseRes>> requestReportBackground(String userNo,String sceneId,String appId,
        String projectId,String backgroundUrl){
        ArrayMap<String, String> params = new ArrayMap();
        if (!TextUtils.isEmpty(backgroundUrl)) {
            params.put("backgroundUrl", backgroundUrl);
        }
        return apiManagerService.requestReportBackground(userNo,sceneId,appId,projectId,getRequestBody(params)).flatMap(it -> Observable.just(it));
    }

    public Observable<BaseResponse<BaseRes>> requestReportAction(String userNo,String sceneId,String appId,
        String projectId,String action){
        ArrayMap<String, String> params = new ArrayMap();
        if (!TextUtils.isEmpty(action)) {
            params.put("action", action);
        }
        return apiManagerService.requestReportAction(userNo,sceneId,appId,projectId,getRequestBody(params)).flatMap(it -> Observable.just(it));
    }
