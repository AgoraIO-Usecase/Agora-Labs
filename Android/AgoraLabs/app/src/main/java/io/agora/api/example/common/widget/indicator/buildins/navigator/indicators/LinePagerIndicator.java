package io.agora.api.example.common.widget.indicator.buildins.navigator.indicators;
...
import io.agora.api.example.common.widget.indicator.FragmentContainerHelper;
import io.agora.api.example.common.widget.indicator.buildins.ArgbEvaluatorHolder;
...
public class LinePagerIndicator extends View implements IPagerIndicator {
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        ...
        if (mMode == MODE_MATCH_EDGE) {
            leftX = current.left + mXOffset;
            nextLeftX = next.left + mXOffset;
            rightX = current.right - mXOffset;
            nextRightX = next.right - mXOffset;
        } else if (mMode == MODE_WRAP_CONTENT) {
            leftX = current.contentLeft + mXOffset;
            nextLeftX = next.contentLeft + mXOffset;
            rightX = current.contentRight - mXOffset;
            nextRightX = next.contentRight - mXOffset;
        } else {    // MODE_EXACTLY
            leftX = current.left + (current.width() - mLineWidth) / 2;
            nextLeftX = next.left + (next.width() - mLineWidth) / 2;
            rightX = current.left + (current.width() + mLineWidth) / 2;
            nextRightX = next.left + (next.width() + mLineWidth) / 2;
        }
